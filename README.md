...
**The Wire accepts an inference server on another machine.** Upstream requires a numeric loopback endpoint (`127.0.0.1` or `::1`) and rejects everything else, so that "local model" can guarantee your data never leaves the computer. This fork lifts the host restriction — `http://192.168.1.50:11434` is accepted — because a homelab commonly runs Ollama on a different box from the dashboard, and a container's own loopback is not the host's. Every other part of the check is unchanged: HTTP or HTTPS only, no credentials in the URL, no query string or fragment, and only `/`, `/v1`, or `/api` as a path. `localhost` is still pinned to `127.0.0.1` so the default cannot be redirected by a hosts file. Requests do not follow redirects. The reasoning is recorded in [docs/FORK_NOTES.md](docs/FORK_NOTES.md).

Be deliberate about what that means. Pointing this at another machine sends your newsletter and article text to that machine, so it is only as private as the box you choose and the network between them. Keep it on hardware you control; a loopback address remains the only setting that guarantees nothing leaves the computer. Local models handle curation, summaries, and newsletters; public Mention discovery continues through the regular news collectors without AI web-search tools.

For processing entirely on one computer, also disable remote forwarding such as [LM Studio's LM Link](https://lmstudio.ai/docs/developer/core/lm-link) in the model runtime. The dashboard cannot inspect or control how another application routes requests internally. A local model must be capable of following the JSON extraction instructions; model failures are reported without fabricating stories.
